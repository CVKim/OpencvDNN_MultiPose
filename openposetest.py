...
parser.add_argument('--image_file', default='multiple.jpeg',
                    help='Input image')


args = parser.parse_args()
# image file path
paths = './input/images/*.jpg'
# image buffer list
imgs = glob.glob(paths) 

# model & config file / pretraing
protoFile = "./model/coco/coco.prototxt"
weightsFile = "./model/coco/coco.caffemodel"
...
def getKeypoints(probMap, threshold=0.1):
    ...
    for cnt in contours:
        ...
        # 최대값, 위치
        (_, maxconf, _, maxpoint) = cv2.minMaxLoc(maskedProbMap)
        keypoints.append(maxpoint + (probMap[maxpoint[1], maxpoint[0]], ))
...
for f in imgs:
    ...
    output = net.forward()
    print('net forward processing time = {}'.format(time.time() - t))
    ...
    t_time, _ = net.getPerfProfile()            
    label = 'Inference Time : %.2f ms ' % (t_time * 1000.0 / cv2.getTickFrequency())
    cv2.putText(frameClone, label, (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
